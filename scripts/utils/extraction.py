...
def process_pdfs_batch(permalinks: List[str]) -> dict:
    ...
    try:
        ...
        pdf_paths = [f"{os.getenv('AWS_BUCKET_NAME')}/raw/ravelry/{p}/{p}.pdf" for p in permalinks]
        ...
        for doc in documents:
            try:
                ...
                s3_client.put_object(Bucket=os.getenv("AWS_BUCKET_NAME"), Key=f"processed/ravelry/{permalink}/{permalink}.txt", Body=results[permalink])
...
async def process_html_file(permalink: str) -> tuple[str, str]:
    ...
    try:
        ...
        html_file = s3_client.get_object(Bucket=os.getenv("AWS_BUCKET_NAME"), Key=html_key)
        text = html_file.get("Body").read().decode("utf-8")
        
        # Trafilatura is synchronous but lightweight enough to not block
        pattern = trafilatura.extract(text, include_comments=False, favor_precision=True, deduplicate=True)

        # Write to s3
        processed_html_key = f"processed/ravelry/{permalink}/{permalink}.txt"
        s3_client.put_object(Bucket=os.getenv("AWS_BUCKET_NAME"), Key=processed_html_key, Body=pattern)
