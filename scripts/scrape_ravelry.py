...
RAVELRY_AUTH = (os.getenv("RAVELRY_PERSONAL_USERNAME"), os.getenv("RAVLERY_PASSWORD_KEY"))
...
async def fetch_patterns(client: httpx.AsyncClient, page: int, query: str) -> List[Dict]:
    """
    ...
    try:
        ...
        for pattern in json_patterns:
            pattern_response = await client.get(
                RAVELRY_API_URL + f"/patterns/{pattern['id']}.json",
                auth=RAVELRY_AUTH
            )
            pattern_response.raise_for_status()
            pattern_json = pattern_response.json()["pattern"]
            available_languages = [language["code"] for language in pattern_json["languages"]]
